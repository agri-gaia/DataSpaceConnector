...
import static org.eclipse.dataspaceconnector.minio.minio.core.MinioClientProviderConfiguration.DEFAULT_AWS_ASYNC_CLIENT_THREAD_POOL_SIZE;
...
public class MinioCoreExtension implements ServiceExtension {
    ...
    @Provider
    public MinioClientProvider awsClientProvider(ServiceExtensionContext context) {
        monitor.info("Some information on Client Provider");
        ...
        var configuration = MinioClientProviderConfiguration.Builder.newInstance()
                .credentialsProvider(createCredentialsProvider(context))
                .endpointOverride(endpointOverride)
                .threadPoolSize(threadPoolSize)
                .build();

        return new MinioClientProviderImpl(configuration);
    }
    ...
    private AwsCredentialsProvider createCredentialsProvider(ServiceExtensionContext context) {
        ...
        System.out.println("Creating Credentialsprovider: " + accessKey + ", " + secretKey);
        monitor.info("Creating Credentialsprovider: " + accessKey + ", " + secretKey);
