...
public class MinioClientProviderImpl implements MinioClientProvider {
    ...
    private final MinioClientProviderConfiguration configuration;
    ...
    public MinioClientProviderImpl(MinioClientProviderConfiguration configuration) {
        ...
        System.out.println("MinioClientProviderImpl: " + configuration.getEndpointOverride().toString());
    }

    @Override
    public S3Client s3Client(String region, SecretToken token) {
        if (token instanceof MinioTemporarySecretToken) {
            var temporary = (MinioTemporarySecretToken) token;
        ...
        } else if (token instanceof MinioSecretToken) {
            var secretToken = (MinioSecretToken) token;
    ...
    @Override
    public S3Client s3Client(String region, SecretToken token, String endpoint) {
        if (token instanceof MinioTemporarySecretToken) {
            var temporary = (MinioTemporarySecretToken) token;
            var credentials = AwsSessionCredentials.create(temporary.getAccessKeyId(), temporary.getSecretAccessKey(), temporary.getSessionToken());
            return createS3ClientWithEndpoint(credentials, region, endpoint);
        } else if (token instanceof MinioSecretToken) {
            var secretToken = (MinioSecretToken) token;
            var credentials = AwsBasicCredentials.create(secretToken.getAccessKeyId(), secretToken.getSecretAccessKey());
            return createS3ClientWithEndpoint(credentials, region, endpoint);
    ...
    @Override
    public BasicAWSCredentials admin_credentials() {
        System.out.println("MinioClientProviderImpl: " + credentialsProvider.resolveCredentials().accessKeyId() + credentialsProvider.resolveCredentials().secretAccessKey());
        return new BasicAWSCredentials(credentialsProvider.resolveCredentials().accessKeyId(), credentialsProvider.resolveCredentials().secretAccessKey());
    }

    @Override
    public String getBaseEndpointOverride(){
        System.out.println("MinioClientProviderImpl: " + configuration.getEndpointOverride().toString());
        return configuration.getEndpointOverride().toString();
    }
    ...
    private S3Client createS3ClientWithEndpoint(AwsCredentials credentials, String region, String endpoint) {
        System.out.println(credentials.accessKeyId() + ", " + credentials.secretAccessKey());
        var credentialsProvider = StaticCredentialsProvider.create(credentials);
        var builder = S3Client.builder()
                .credentialsProvider(credentialsProvider)
                .region(Region.of(region));

        handleBaseEndpointOverrideWithEndpoint(builder, endpoint);
    ...
    private void handleBaseEndpointOverrideWithEndpoint(S3BaseClientBuilder<?, ?> builder, String endpoint) {
